...
sys.modules['faster_whisper.audio'] = __import__("aTrain.audio",fromlist=[None]) 
...
def decode_audio(file: str, sampling_rate: int = 16000, split_stereo = False):
    ffmpeg_path = get_ffmpeg()
    try:
        stream = ffmpeg.input(file, threads=0)
        stream = ffmpeg.output(stream,"-", format="s16le", acodec="pcm_s16le", ac=1, ar=sampling_rate)
        if platform.system()=="Windows":
