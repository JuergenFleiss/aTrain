...
def get_timestamp():
    """
    Returns a timestamp string with the current date and time in 24-hour format.
    Example format: '2023-03-17 15:24:30'
    """
...
def create_output_files(result, speaker_detection, file_directory, orig_filename, k=False):
        if k is not False:
            ts = create_json_file(result, file_directory, k)
        else:
            ts = create_json_file(result, file_directory)
        try:
...
def create_json_file(result, file_directory, k=False):
    if k is False:
        dt = get_timestamp()
        dt = dt.replace(" ","_").replace(":","").replace("-","")
        output_file_text = os.path.join(file_directory,f"transcription{dt}.json")
    else: 
        output_file_text = os.path.join(file_directory,f"transcription{k}.json")
