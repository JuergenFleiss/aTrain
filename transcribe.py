...
def handle_transcription(file_id):
    try:
        ...
        segments = split_audio(prepared_file, file_directory, segment_duration=1500)  # Split the audio file into segments of 30 minutes each
        log_metadata_to_file(segments=segments)
        segment_log = "C:\\Users\\dower\\Documents\\segments.json"
        transcripts_path = "C:\\Users\\dower\\Documents\\transcripts.json"
        transcripts = {}
        with open(segment_log, 'w') as f:
            json.dump(segments, f)
        for k, segment in enumerate(segments):
            print(str(f'{segment}{k}'))
            name = Path(segment).name
            prepared_file = os.path.join(file_directory, name + ".wav")
            print('transcribing segment:', name)
            for step in transcribe(segment, model, language, speaker_detection, num_speakers, device, compute_type):
                ...
                yield response
            print(f'transcription done for segment:{name}\nwriting output.')
            log_metadata_to_file(speaker=speaker_detection, file_id=name, prepared_file=prepared_file, filename=filename)
            transcripts.update(create_output_files(step["result"], speaker_detection, file_directory, filename, k))
            try:
